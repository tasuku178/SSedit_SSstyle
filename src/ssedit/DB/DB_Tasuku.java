...
//		}
			...
			try {
				...
				if (driver.isEmpty()) {
					driver = "postgresql";
					FrontEnd_Tasuku.driverCombo.setSelectedIndex(1);
...
//					System.out.println(Common.has(GlobalEnv.USER_HOME + GlobalEnv.OS_FS + GlobalEnv.configFile, "db"));
				...
				try {
					...
					while (result_table.next()) {// 次のテーブルが存在しなくなるまで
						...
						// テーブル
						FrontEnd_Tasuku.str_attribute = result_table.getString("TABLE_NAME");
						if (!FrontEnd_Tasuku.str_attribute.startsWith("pg_")
								&& !FrontEnd_Tasuku.str_attribute.startsWith("sql_") && !FrontEnd_Tasuku.str_attribute.startsWith("SQLITE")) {
							GlobalEnv.table_array.add(FrontEnd_Tasuku.str_attribute);
							ResultSet result_attribute = dmd.getColumns(null,
									null, FrontEnd_Tasuku.str_attribute, "%");// テーブル上のスキーマを取得
							...
							GlobalEnv.tabledata.put(FrontEnd_Tasuku.str_attribute, attributedata);// スキーマの載った葉をGlobalEnv.tabledataに載せる
						}
					}
...
//									String attribute = current.toString().substring(0, current.toString().indexOf("(") - 1);
					...
					ImageIcon icon2 = new ImageIcon(FrontEnd_Tasuku.get_execDir()
							+ GlobalEnv.OS_FS + "image" + GlobalEnv.OS_FS
							+ "vector4.png");
					ImageIcon icon3 = new ImageIcon(FrontEnd_Tasuku.get_execDir()
							+ GlobalEnv.OS_FS + "image" + GlobalEnv.OS_FS
							+ "icon.png");
					...
					FrontEnd_Tasuku.table_scrollpane.getViewport().setView(tree);
					FrontEnd_Tasuku.table_scrollpane.setPreferredSize(new Dimension(550, 1200));
				} finally {
					result_table.close();
				}
				stmt.close();
